        ...
        <div class="card card-flush pt-3 mb-5 mb-xl-10">
          ...
          <div class="card-header">
            ...
            <div class="card-title">
              <h2 class="fw-bolder">{{ $t("Shipment company Types") }}</h2>
            </div>
            <!--begin::Card title-->
            <!--begin::Card toolbar-->
...
export default defineComponent({
  ...
  created() {
    ApiService.get("admin/Campany/ShipmentTypes").then((data) => {
      this.tableDataTypes = data.data.data;
      this.loading = false;
    });
  },
  mounted() {
    setCurrentPageBreadcrumbs(this.$t("type"), [this.$t("cp")]);
  },
  methods: {
    openModelTypeNew() {
      this.isEditType = -1;
    ...
    async submitModelForm() {
      ...
      if (this.isEditType > -1) {
        ...
        await ApiService.post("admin/Campany/ShipmentTypes", this.TypeForm)
          .then((res) => {
            if (res.data.data) {
              this.$toast.success(this.$t("messages.success"));
