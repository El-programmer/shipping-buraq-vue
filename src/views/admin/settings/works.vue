          ...
          <div class="card-header">
            ...
            <div class="card-toolbar">
              <a
                ...
                data-bs-target="#kt_modal_work_item"
                data-bs-toggle="modal"
                @click="openModelWorkNew()"
              >
                {{ $t("addWork") }}
              </a>
            </div>
              ...
              <div class="table-responsive">
                ...
                >
                  ...
                  <tbody class="fw-bold text-gray-800">
                    <tr v-for="(item, i) in tableDataworkFilter" :key="i">
                      <td>
      ...
      <div class="modal-dialog modal-dialog-centered mw-650px">
        ...
        <div class="modal-content">
          ...
          <div id="kt_modal_add_Country_header" class="modal-header">
            <!--begin::Modal title-->
            <h2 v-if="isEditWork > -1" class="fw-bolder">
              {{ $t("editWork") }}
            </h2>
            <h2 v-else class="fw-bolder">{{ $t("addWork") }}</h2>
            <!--end::Modal title-->

            <!--begin::Close-->
            <div
              id="kt_modal_work_item_close"
              ...
              >
                <div class="fv-row mb-7">
                  ...
                  <el-form-item prop="name">
                    <el-input v-model="WorkForm.name" type="text" />
                  </el-form-item>
                  <span v-if="WorkErrors['name']" class="text-danger">{{
                    WorkErrors["name"][0]
                  }}</span>
                </div>
              </div>
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      tableDatawork: [],
      WorkformModelRef: (ref < null) | (HTMLFormElement > null),
      WorkForm: {},
      WorkErrors: [],
      isEditWork: -1,
      transObject: {},
  ...
  created() {
    ApiService.get("admin/works").then((data) => {
      this.tableDatawork = data.data.data;
      this.loading = false;
    });
  },
  mounted() {
    setCurrentPageBreadcrumbs(this.$t("work"), [this.$t("cp")]);
  },
  methods: {
    openModelWorkNew() {
      this.isEditWork = -1;
    ...
    openModelWorkEdit(i) {
      this.isEditWork = i;
      this.WorkForm = this.tableDatawork[i];
    },
    async submitModelForm() {
      this.loading = true;
      this.WorkErrors = [];
      if (this.isEditWork > -1) {
        await ApiService.put(
          "admin/works/" + this.tableDatawork[this.isEditWork].id,
          this.WorkForm
        )
          .then((res) => {
            hideModal("kt_modal_work_item");
            this.$toast.success(this.$t("messages.success"));
            this.tableDatawork[this.isEditWork] = res.data.data;
          })
          .catch((error) => {
            this.WorkErrors = ApiService.errors;
          });
      } else {
        await ApiService.post("admin/works", this.WorkForm)
          .then((res) => {
            if (res.data.data) {
              this.$toast.success(this.$t("messages.success"));
              this.tableDatawork.push(res.data.data);
              hideModal("kt_modal_work_item");
            }
            this.loading = false;
          })
    ...
    async deleteWork(i) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ApiService.delete("admin/works/" + this.tableDatawork[i].id).then(
            (res) => {
              this.tableDatawork.splice(i, 1);
              Swal.fire(
  ...
  computed: {
    tableDataworkFilter() {
      return this.tableDatawork.filter(
        (data) =>
          !this.search ||
