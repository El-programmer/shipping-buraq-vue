        ...
        <div class="card card-flush pt-3 mb-5 mb-xl-10">
          ...
          <div class="card-header">
            ...
            <div class="card-title">
              <h2 class="fw-bolder">{{ $t("orderTypes") }}</h2>
            </div>
            <!--begin::Card title-->
            <!--begin::Card toolbar-->
              ...
              <div class="table-responsive">
                ...
                >
                  ...
                  <tbody class="fw-bold text-gray-800">
                    <tr v-for="(item, i) in tableDataOrderTypes" :key="i">
                      <td>
                      ...
                      <td class="text-end">
                        <a
                          ...
                          data-bs-target="#kt_modal_order_type_item"
                          data-bs-toggle="modal"
                          @click="openModelOrderTypeEdit(i)"
                        >
      ...
      <div class="modal-dialog modal-dialog-centered mw-650px">
        ...
        <div class="modal-content">
          ...
          <div id="kt_modal_add_Country_header" class="modal-header">
            <!--begin::Modal title-->
            <h2 v-if="isEditOrderType > -1" class="fw-bolder">
              {{ $t("editOrderType") }}
            </h2>
            <h2 v-else class="fw-bolder">{{ $t("addOrderType") }}</h2>
            <!--end::Modal title-->
              ...
              >
                <div class="fv-row mb-7">
                  ...
                  <el-form-item prop="name">
                    <el-input v-model="OrderTypeForm.name" type="text" />
                  </el-form-item>
                  <span v-if="OrderTypeErrors['name']" class="text-danger">{{
                    OrderTypeErrors["name"][0]
                  }}</span>
                </div>
                <div class="d-flex flex-column mb-7 fv-row">
                  ...
                  <select v-model="OrderTypeForm.status" class="form-select">
                    <option value="1">{{ $t("yes") }}</option>
                    <option value="0">{{ $t("no") }}</option>
                  </select>
                  <span v-if="OrderTypeErrors['status']" class="text-danger">{{
                    OrderTypeErrors["status"][0]
                  }}</span>
                </div>
              </div>
              <!--end::Scroll-->
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      tableDataOrderTypes: [],
      OrderTypeformModelRef: (ref < null) | (HTMLFormElement > null),
      OrderTypeForm: {},
      OrderTypeErrors: [],
      isEditOrderType: -1,
      transObject: {},
  ...
  created() {
    ApiService.get("admin/orderTypes").then((data) => {
      this.tableDataOrderTypes = data.data.data;
      this.loading = false;
    });
  },
  mounted() {
    setCurrentPageBreadcrumbs(this.$t("orderTypes"), [this.$t("cp")]);
  },
  methods: {
    openModelOrderTypeNew() {
      this.isEditOrderType = -1;
      this.OrderTypeForm = {};
    },
    openModelOrderTypeEdit(i) {
      this.isEditOrderType = i;
      this.OrderTypeForm = this.tableDataOrderTypes[i];
    },
    async submitModelForm() {
      this.loading = true;
      this.OrderTypeErrors = [];
      if (this.isEditOrderType > -1) {
        await ApiService.put(
          "admin/orderTypes/" +
            this.tableDataOrderTypes[this.isEditOrderType].id,
          this.OrderTypeForm
        )
          .then((res) => {
            hideModal("kt_modal_order_type_item");
            this.$toast.success(this.$t("messages.success"));
            this.tableDataOrderTypes[this.isEditOrderType] = res.data.data;
          })
          .catch((error) => {
            this.OrderTypeErrors = ApiService.errors;
          });
      } else {
        await ApiService.post("admin/orderTypes", this.OrderTypeForm)
          .then((res) => {
            if (res.data.data) {
              this.$toast.success(this.$t("messages.success"));
              this.tableDataOrderTypes.push(res.data.data);
              hideModal("kt_modal_order_type_item");
            }
            this.loading = false;
          })
    ...
    async deleteOrderType(i) {
      Swal.fire({
        title:
          this.$t("messages.areSureDeleted") +
          " ? ( " +
          this.tableDataOrderTypes[i].name +
          " )",
        ...
        if (result.isConfirmed) {
          ApiService.delete(
            "admin/orderTypes/" + this.tableDataOrderTypes[i].id
          ).then((res) => {
            this.tableDataOrderTypes.splice(i, 1);
            Swal.fire(
