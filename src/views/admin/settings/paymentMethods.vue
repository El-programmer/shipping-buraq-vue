  ...
  <div id="kt_content" class="content flex-row-fluid">
    ...
    <div class="d-flex flex-column flex-lg-row">
      ...
      >
        ...
        <div v-if="tableDataItems.length == 0">
          <KTModalCard
            :button-text="$t('Add New')"
            :description="$t('noData')"
            :title="$t('Welcome')"
            image="/media/illustrations/sigma-1/n18.png"
            modal-id="kt_modal_Blog_item"
          ></KTModalCard>
        </div>
        <div class="card card-flush pt-3 mb-5 mb-xl-10" v-else>
          <!--begin::Card header-->
          <div class="card-header">
            <!--begin::Card title-->
            <div class="card-title">
              <h2 class="fw-bolder">{{ $t("pymentMethods") }}</h2>
            </div>
            <!--begin::Card title-->
            <!--begin::Card toolbar-->
                ...
                >
                  ...
                  <thead>
                    ...
                    >
                      ...
                      <th class="min-w-150px">{{ $t("status") }}</th>
                      <th class="min-w-125px">{{ $t("mode") }}</th>
                  ...
                  <tbody class="fw-bold text-gray-800">
                    <tr v-for="(item, i) in tableDataItems" :key="i">
                      <td>
                        <label class="w-150px">{{ item.module }}</label>
      ...
      <div class="modal-dialog modal-dialog-centered mw-650px">
        ...
        <div class="modal-content">
          ...
          <div id="kt_modal_add_Country_header" class="modal-header">
            ...
            <h2 v-else class="fw-bolder">{{ $t("addPayment") }}</h2>
            <!--end::Modal title-->
            ...
            <div class="modal-body py-10 px-lg-17">
              ...
              >
                <div class="fv-row mb-7">
                  ...
                  <span v-if="pymentError['name']" class="text-danger">{{
                    pymentError["name"][0]
                  }}</span>
                </div>
                ...
                <div class="fv-row mb-7">
                  ...
                  <el-form-item prop="name">
                    <el-input v-model="ItemForm.module" type="text" />
                  </el-form-item>
                  <span v-if="pymentError['module']" class="text-danger">{{
                    pymentError["module"][0]
                  }}</span>
                </div>
                ...
                <div class="d-flex flex-column mb-7 fv-row">
                  <label class="fs-6 fw-bold mb-2">
                    <span class="required">{{ $t("mode") }}</span>
                  </label>
                  <select v-model="ItemForm.mode" class="form-select">
                    <option value="1">{{ $t("live") }}</option>
                    <option value="0">{{ $t("sandbox") }}</option>
                  </select>
                </div>
                ...
                <div class="fv-row mb-7">
                  ...
                  <el-form-item prop="name">
                    <el-input v-model="ItemForm.live_user" type="text" />
                  </el-form-item>
                  <span v-if="pymentError['live_user']" class="text-danger">{{
                    pymentError["live_user"][0]
                  }}</span>
                </div>
                <div class="fv-row mb-7">
                  <label class="required fs-6 fw-bold mb-2"
                  ...
                  <el-form-item prop="name">
                    <el-input v-model="ItemForm.live_secret" type="text" />
                  </el-form-item>
                  <span v-if="pymentError['live_secret']" class="text-danger">{{
                    pymentError["live_secret"][0]
                  }}</span>
                </div>
                ...
                <div class="fv-row mb-7">
                  ...
                  <el-form-item prop="name">
                    <el-input v-model="ItemForm.sandbox_user" type="text" />
                  </el-form-item>
                  <span
                    v-if="pymentError['sandbox_user']"
                    class="text-danger"
                    >{{ pymentError["sandbox_user"][0] }}</span
                  >
                </div>
                <div class="fv-row mb-7">
                  <label class="required fs-6 fw-bold mb-2"
                  ...
                  <el-form-item prop="name">
                    <el-input v-model="ItemForm.sandbox_secret" type="text" />
                  </el-form-item>
                  <span
                    v-if="pymentError['sandbox_secret']"
                    class="text-danger"
                    >{{ pymentError["sandbox_secret"][0] }}</span
                  >
                </div>
              </div>
...
export default defineComponent({
  ...
  created() {
    ApiService.get("admin/PaymentMethods")
      .then((data) => {
  ...
  mounted() {
    setCurrentPageBreadcrumbs(this.$t("pymentMethods"), [this.$t("cp")]);
  },
  methods: {
    openModelNew() {
      this.isEdit = -1;
    ...
    async submitModelForm() {
      ...
      if (this.isEdit > -1) {
        await ApiService.put(
          "admin/PaymentMethods/" + this.tableDataItems[this.isEdit].id,
          this.ItemForm
        )
          .then((res) => {
          ...
          .catch(() => {
            this.pymentError = ApiService.errors;
          });
      } else {
        await ApiService.post("admin/PaymentMethods", this.ItemForm)
          .then((res) => {
            if (res.data.data) {
              this.$toast.success(this.$t("messages.success"));
    ...
    async deleteItem(i) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ApiService.delete(
            "admin/PaymentMethods/" + this.tableDataItems[i].id
          ).then((res) => {
