<template>
  ...
  <div id="kt_modal_create_order" ref="createAccountModalRef" aria-hidden="true" class="modal fade" tabindex="-1">
    <!--begin::Modal dialog-->
        ...
        <div class="modal-body scroll-y ">
          <div class="row">
            ...
            <div>
              ...
              <div class="row w-100 row-cols-1 row-cols-sm-2 row-cols-md-2 row-cols-lg-4 justify-content-around">
                <div class="" v-for="item in paths" :key="item.id">
                  <label class="btn btn-outline btn-outline-dashed btn-outline-default d-flex text-start p-6"
                    ...
                    <!--end::Radio button-->
                    <span class="ms-5">
                      <span class="fs-6 fw-bolder mb-1 d-block">{{ item.title }}</span>
...
import Index from "@/views/site/index-old/index.vue";
...
import ReceiverAddress from "@/views/admin/orders/receiverAddress.vue";
  ...
  methods: {
    ...
    async onComplete() {
      ...
      var res = await this.apiOrder.store(this.order, (res) => {
        ...
        hideModal('kt_modal_create_order');
      }, false)
      this.loading = false;
