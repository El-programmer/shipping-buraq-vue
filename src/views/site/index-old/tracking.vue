...
<path
    ...
    xmlns="http://www.w3.org/2000/svg"></path>
                        ...
                        <div class="fw-bold fs-6 text-gray-400">{{$t('count')}} {{order.service.name}}</div>
                      </div>
                    </div>
                  </div>
              ...
              <div class="card-body pt-5">
                <div  v-if="order.service && order.service.name">
                  <div class="d-flex flex-stack align-items-center flex-wrap"><h4
                      class="text-gray-400 fw-bold mb-0 pe-2">{{ order.service.name}}</h4></div>
    ...
    async serchOrder() {
      ...
      try {
        this.order.code = this.order.code
            .replace("https://mollerlogistic.com/tracking/", "")
            .substr(0, 13);
