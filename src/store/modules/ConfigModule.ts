...
export default class ConfigModule extends VuexModule implements StoreInfo {
  ...
  @Mutation
  [Mutations.GETSETTING](payload = ""): void {
    const setting = JSON.parse(
      localStorage.getItem("Settings") ||
        '{ "brand": "Logistic Company","appTimezone": "AFRICA/CAIRO",  "head_logo_white": "/media/logos/logo-5.png", "head_logo_colored": "logo-5.png", "site_main_image": "pattern-1.jpg"}'
    );

    if (payload == "") return setting;
    else if (setting[payload]) return setting[payload];
    else return setting;
  }
}
