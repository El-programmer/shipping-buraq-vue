  ...
  <div class="d-flex align-items-center ms-1 ms-lg-3">
    ...
    >
      ...
      >
        ...
        <!--begin::Tabs-->
        <ul
            class="nav nav-line-tabs nav-line-tabs-2x nav-stretch
          fw-bold
          px-9
          d-none"
        >
      ...
      <div class="py-3 text-center border-top">
        <router-link
            :to="{name:'site-notifications-list'}"
            class="btn btn-color-gray-600 btn-active-color-primary" href="#">
          {{$t('View All')}}
...
export default defineComponent({
  ...
  methods:{
    load(){
      if (JwtService.isCustomer()) {
        ApiService.get('/site/customer/Notification/unread').then((res) => {
    ...
    showList(){
      if (this.hasNew){
        ApiService.get('/site/customer/notification/read/all').finally(()=>{
          this.hasNew = false;
        });
      }
    }
